...
class SellerDraftTabProductProvider extends ChangeNotifier {
  ...
  /// Fetch Products
  void fetchDraftProducts(BuildContext context) async {
    ...
    try {
      var response = await SellerProductRepository.allSellerProductsUsingPagination(context, ProductStatus.draft,currentPage);
      List<Product> newProducts = response.getAllProducts.where((d) => d.status == ProductStatus.draft).toList();
