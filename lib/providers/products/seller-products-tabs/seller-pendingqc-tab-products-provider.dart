...
class SellerPendingQCTabProductProvider extends ChangeNotifier {
  ...
  /// Fetch Products
  void fetchPendingQCProducts(BuildContext context) async {
    ...
    try {
      var response = await SellerProductRepository.allSellerProductsUsingPagination(context,ProductStatus.pendingqc, currentPage);
      List<Product> newProducts = response.getAllProducts.where((d) => d.status == ProductStatus.pendingqc).toList();
  ...
  void autoLoadMoreIfNeeded(BuildContext context, double screenHeight, double itemHeight) {
    if (!isLoading && hasMoreData) {
      ...
      if (totalListHeight < screenHeight) {
        fetchPendingQCProducts(context);
      }
    }
  }
