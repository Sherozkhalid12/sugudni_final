...
class SellerViolationTabProductProvider extends ChangeNotifier {
  ...
  /// Fetch Products
  void fetchViolationProducts(BuildContext context) async {
    ...
    try {
      var response = await SellerProductRepository.allSellerProductsUsingPagination(context,ProductStatus.violation, currentPage);
      List<Product> newProducts = response.getAllProducts.where((d) => d.status == ProductStatus.violation).toList();
  ...
  void autoLoadMoreIfNeeded(BuildContext context, double screenHeight, double itemHeight) {
    if (!isLoading && hasMoreData) {
      ...
      if (totalListHeight < screenHeight) {
        fetchViolationProducts(context);
      }
    }
  }
