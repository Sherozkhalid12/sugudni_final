...
class ChatSocketProvider extends ChangeNotifier {
  ...
  Future<void> fetchThreads(BuildContext context) async {
    try {
      ...
      sellerThreads =
          await SellerMessagesRepository.getThreadsForSeller(context);
      customPrint(
          "Threads data=================================${sellerThreads!.threads}");
      notifyListeners();
    } catch (e) {
  ...
  getChatHistory(
      BuildContext context, String receiverId, String senderId) async {
    isLoading = true;
    notifyListeners();
    try {
      chatHistoryResponse = await SellerMessagesRepository.getChatHistory(
  ...
  void sendMessage(String receiverId, String senderId) {
    if (messageController.text.isNotEmpty) {
      ...
      if (socket == null || !socket!.connected) {
        customPrint(
            'Socket not connected, message will be sent when socket connects');
        ...
        Future.delayed(const Duration(milliseconds: 500), () {
          if (socket != null && socket!.connected) {
            ...
            customPrint(
                'Failed to connect socket, removing optimistic message');
  ...
  void _sendMessageToSocket(
    ...
    socket!.emit('newThread', {'receiverid': receiverId});
  }
  ...
  void sendMedia(String receiverId, String senderId, BuildContext context) {
    ...
    if (socket == null || !socket!.connected) {
      customPrint(
          'Socket not connected, media will be sent when socket connects');
      // Wait a bit and retry (socket should be initialized by the chat screen)
      Future.delayed(const Duration(milliseconds: 500), () {
        if (socket != null && socket!.connected) {
  ...
  void _sendMediaToSocket(
      String receiverId, String senderId, BuildContext context) {
    if (socket == null || !socket!.connected) {
  ...
  Future<void> markAsUnread(
      String userid, String otherUserid, BuildContext context) async {
