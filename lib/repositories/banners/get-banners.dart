...
import '../../models/banners/GetBannerListResponse.dart';

class BannersRepository{
  static Future<GetBannerListResponse> allBannersPagination(BuildContext context,int page) async {
    bool isConnected = await checkInternetConnection();
    if(!isConnected){
      throw 'Please check your internet and try again.';
    }
    final response = await ApiClient.getRequest("${ApiEndpoints.sale}?page=$page",headers:await ApiClient.bearerHeader).timeout(const Duration(seconds: 30),onTimeout: (){
    ...
    if(response.statusCode==201){
      return _handleResponse(response, (data) => GetBannerListResponse.fromJson(body));
    }else{
