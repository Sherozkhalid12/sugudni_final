...
class WishlistRepository{
  static Future<AddToWishlistResponse> addProductToWishlist(AddToWishlistModel model,BuildContext context) async {
    bool isConnected = await checkInternetConnection();
    if(!isConnected){
      throw 'Please check your internet and try again.';
    }
    final response = await ApiClient.patchRequest(ApiEndpoints.wishlist,model.toJson()).timeout(const Duration(seconds: 30),onTimeout: (){
    ...
    if(response.statusCode==201){
      return _handleResponse(response, (data) => AddToWishlistResponse.fromJson(body));
    }else{
  ...
  static Future<GetAllWishlistResponseModel> getWishlistProducts(BuildContext context) async {
    bool isConnected = await checkInternetConnection();
    if(!isConnected){
      throw 'Please check your internet and try again.';
    }
    final response = await ApiClient.getRequest(ApiEndpoints.wishlist).timeout(const Duration(seconds: 30),onTimeout: (){
      throw 'Request timed out. Please try again.';
    });
    final body=jsonDecode(response.body);
    final statusCode=response.statusCode;
    ...
    customPrint("Status code:================$statusCode");
    if(response.statusCode==201){
      return _handleResponse(response, (data) => GetAllWishlistResponseModel.fromJson(body));
    }else{
      final error=body['error'];
      showSnackbar(context, error.toString(),color: redColor);
      throw error;
  ...
  static Future<RemoveFromWishlistResponseModel> removeFromWishlist(AddToWishlistModel model,BuildContext context) async {
    bool isConnected = await checkInternetConnection();
    if(!isConnected){
      throw 'Please check your internet and try again.';
    }
    final response = await ApiClient.deleteRequestWithBody(ApiEndpoints.wishlist,model.toJson()).timeout(const Duration(seconds: 30),onTimeout: (){
      throw 'Request timed out. Please try again.';
    });
    final body=jsonDecode(response.body);
    final statusCode=response.statusCode;
    ...
    customPrint("Status code:================$statusCode");
    if(response.statusCode==201){
      return _handleResponse(response, (data) => RemoveFromWishlistResponseModel.fromJson(body));
    }else{
      final error=body['error'];
      showSnackbar(context, error.toString(),color: redColor);
      throw error;
